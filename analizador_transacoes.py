...
def carrega (nome_do_arquivo):
    try:
        ...
        print(f"Erro no carregamento de arquivos: {e}")
...
def salva(nome_do_arquivo, conteudo):
    try:
        with open(nome_do_arquivo, "w", encoding="utf-8") as arquivo:
         ...
         print(f"Erro no salvamento de arquivos: {e}")

def analisar_transacao(lista_transacoes):
    print("1. Executando análise de transações.")
...
for uma_transacao in transacoes_analisadas["transacoes"]:
    if uma_transacao["status"] == "Possível Fraude":
        ...
        salva(f"transacao-{id_transacao}-{produto_transacao}.txt", f"Status: {status_transacao}\nParecer: {um_parecer}\nRecomendação: {recomendacao}")